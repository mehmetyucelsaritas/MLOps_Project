...
def fix_broken_words(text):
    ...
    text = re.sub(r'\b(?:[a-zA-Z]\s){1,4}[a-zA-Z]\b',
                  lambda m: m.group(0).replace(" ", ""), text)

    # fix: 'ont he' → 'on the' 
    text = re.sub(r'\b(\w{1,3})\s+(\w{1,3})\b', lambda m: m.group(1) + m.group(2) if len(m.group(1) + m.group(2)) <= 5 else m.group(0), text)
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            body_words = [
                ...
                if margin < w["top"] < (page_height - margin)
            ]
